...
RUN wget 'https://go.dev/dl/go1.26rc1.linux-amd64.tar.gz' && \
	mkdir /go1.26 && \
	tar x -C /go1.26 -f go1.26rc1.linux-amd64.tar.gz && \
...
COPY --from=downloader /go1.26/* /usr/local

RUN DEBIAN_FRONTEND=noninteractive apt-get update && \
    apt-get install -qyy pkg-config gcc-mingw-w64-x86-64 g++-mingw-w64-x86-64 mingw-w64-x86-64-dev && \
    apt-get clean
