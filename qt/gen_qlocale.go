...
func (this *QLocale) MeasurementSystem() QLocale__MeasurementSystem {
	...
	return (QLocale__MeasurementSystem)(ret)
}
...
func QLocale_CountriesForLanguage(lang QLocale__Language) []QLocale__Country {
	...
	ret := make([]QLocale__Country, int(_out_len))
	_outCast := (*[0xffff]C.uintptr_t)(unsafe.Pointer(_out)) // mrs jackson
	for i := 0; i < int(_out_len); i++ {
		ret[i] = (QLocale__Country)(_outCast[i])
	}
	C.free(unsafe.Pointer(_out))
	return ret
}
