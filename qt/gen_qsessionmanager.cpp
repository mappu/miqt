...
void QSessionManager_setRestartCommand(QSessionManager* self, struct miqt_array /* of struct miqt_string */  restartCommand) {
	QStringList restartCommand_QStringList;
	restartCommand_QStringList.reserve(restartCommand.len);
	...
	self->setRestartCommand(restartCommand_QStringList);
}
...
void QSessionManager_setDiscardCommand(QSessionManager* self, struct miqt_array /* of struct miqt_string */  discardCommand) {
	QStringList discardCommand_QStringList;
	discardCommand_QStringList.reserve(discardCommand.len);
	...
	for(size_t i = 0; i < discardCommand.len; ++i) {
		...
		discardCommand_QStringList.push_back(discardCommand_arr_i_QString);
	}
	self->setDiscardCommand(discardCommand_QStringList);
}
...
void QSessionManager_setManagerProperty2(QSessionManager* self, struct miqt_string name, struct miqt_array /* of struct miqt_string */  value) {
	QString name_QString = QString::fromUtf8(name.data, name.len);
	QStringList value_QStringList;
	value_QStringList.reserve(value.len);
	...
	for(size_t i = 0; i < value.len; ++i) {
		...
		value_QStringList.push_back(value_arr_i_QString);
	}
	self->setManagerProperty(name_QString, value_QStringList);
}
