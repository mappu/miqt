...
func main() {
	...
	mainwindow.SetWindowTitle("Qt 5 Translation Example")
...
func retranslate() {
	label.SetText(qt.QLabel_Tr("L&anguage:"))
	upButton.SetText(qt.QPushButton_Tr("&Up"))
	downButton.SetText(qt.QPushButton_Tr("&Down"))
	leftButton.SetText(qt.QPushButton_Tr("&Left"))
	rightButton.SetText(qt.QPushButton_Tr("&Right"))
	exitAction.SetText(qt.QAction_Tr("E&xit"))
	fileMenu.SetTitle(qt.QMenu_Tr("&File"))

	exitKey := qt.NewQKeySequence2(qt.QAction_Tr2("Ctrl+Q", "Quit"))
	defer exitKey.Delete()
