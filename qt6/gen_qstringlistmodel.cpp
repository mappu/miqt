...
public:
	...
	MiqtVirtualQStringListModel(const QList<QString>& strings): QStringListModel(strings) {}
	MiqtVirtualQStringListModel(QObject* parent): QStringListModel(parent) {}
	MiqtVirtualQStringListModel(const QList<QString>& strings, QObject* parent): QStringListModel(strings, parent) {}
...
struct miqt_array /* of struct miqt_string */  QStringListModel_virtualbase_mimeTypes(const void* self) {
	QList<QString> _ret = static_cast<const MiqtVirtualQStringListModel*>(self)->QStringListModel::mimeTypes();
	// Convert QList<> from C++ memory to manually-managed C memory
	struct miqt_string* _arr = static_cast<struct miqt_string*>(malloc(sizeof(struct miqt_string) * _ret.size()));
	for (size_t i = 0, e = _ret.size(); i < e; ++i) {
		QString _lv_ret = _ret[i];
...
struct miqt_array /* of QModelIndex* */  QStringListModel_virtualbase_match(const void* self, QModelIndex* start, int role, QVariant* value, int hits, int flags) {
	QList<QModelIndex> _ret = static_cast<const MiqtVirtualQStringListModel*>(self)->QStringListModel::match(*start, static_cast<int>(role), *value, static_cast<int>(hits), static_cast<Qt::MatchFlags>(flags));
	// Convert QList<> from C++ memory to manually-managed C memory
