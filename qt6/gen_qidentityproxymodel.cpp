...
struct miqt_array /* of QModelIndex* */  QIdentityProxyModel_virtualbase_match(const void* self, QModelIndex* start, int role, QVariant* value, int hits, int flags) {
	QList<QModelIndex> _ret = static_cast<const MiqtVirtualQIdentityProxyModel*>(self)->QIdentityProxyModel::match(*start, static_cast<int>(role), *value, static_cast<int>(hits), static_cast<Qt::MatchFlags>(flags));
	// Convert QList<> from C++ memory to manually-managed C memory
	QModelIndex** _arr = static_cast<QModelIndex**>(malloc(sizeof(QModelIndex*) * _ret.size()));
	for (size_t i = 0, e = _ret.size(); i < e; ++i) {
		_arr[i] = new QModelIndex(_ret[i]);
...
struct miqt_array /* of struct miqt_string */  QIdentityProxyModel_virtualbase_mimeTypes(const void* self) {
	QList<QString> _ret = static_cast<const MiqtVirtualQIdentityProxyModel*>(self)->QIdentityProxyModel::mimeTypes();
	// Convert QList<> from C++ memory to manually-managed C memory
